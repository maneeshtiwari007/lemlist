...
class ProjectController extends Controller
{
    ...
    public function __construct(ProjectRepository $projectRepositery, JobRepository $jobRepositery,UserRepository $userRepositery)
    {
    ...
    public function index(Request $req){
		$arrContent = array();
		$arrContent['title'] = "Projects | Statuscrawl";
		$user = Auth::user();
		$get = $req->input();
        $arrData['get'] = $get;
		$projects = $this->projectRepositery->getAll($get);
		$arrData['getUsers'] = $this->userRepositery->getAllUsers();
		//echo "<pre>";var_dump($projects);exit;
		return view('projects.index',[
                    'projects'=>$projects,
					'user'=>$user,
					'getUsers'=>$arrData['getUsers'],
	...
	public function add(Request $request){
		...
		$arrData['arrContent']['title'] = "Add New Project | Statuscrawl";
	...
	public function getEdit(Request $request, $id){
		$arrData = array();
		$arrData['arrContent'] = array();
		$arrData['arrContent']['title'] = "Edit Project | Statuscrawl";
	...
	public function getProjectView(Request $request, $id){
		$arrData = array();
		$arrData['arrContent'] = array();
		$arrData['arrContent']['title'] = "Project Url | Statuscrawl";
		$user = Auth::user();
	...
	public function getProjectUrl($id){
		if(!empty($id)){
			$arrContent = array();
		    $arrContent['title'] = "Projects Url | Statuscrawl";
			$user = Auth::user();
	...
	public function addkeyword(Request $request,$id){
		if(!empty($id)){
			$arrContent = array();
		    $arrContent['title'] = "Projects Url Keyword | Statuscrawl";
	...
	public function getProjectExactKeywordList(Request $request){
		$user = Auth::user();
		$arrData = array();
        if ($request->ajax() && $request->method() == "POST")
        {
			...
			$arrData['projectId'] = $inputProjectId;
			$projectExactKeywordData = $this->projectRepositery->getProjectExactKeyword($inputProjectId);
			$arrData['projectExactKeywordData'] = $projectExactKeywordData;
			//echo "<pre>";var_dump($arrData['projectExactKeywordData']);exit;
			$arrResponseData = View::make('projects.view-exact-keyword', $arrData);
            return response()->json(array(
                'status' => 200,
                'result' => $arrResponseData->render()
            ));
	...
	public function getProjectPartialKeywordList(Request $request){
		$user = Auth::user();
		$arrData = array();
        if ($request->ajax() && $request->method() == "POST")
        {
			...
			$inputProjectId = $input->projectId;
			$arrData['projectId'] = $inputProjectId;
			$projectPartialKeywordData = $this->projectRepositery->getProjectPartialKeyword($inputProjectId);
			$arrData['projectPartialKeywordData'] = $projectPartialKeywordData;
			$arrResponseData = View::make('projects.view-partial-keyword', $arrData);
            return response()->json(array(
                'status' => 200,
                'result' => $arrResponseData->render()
            ));
