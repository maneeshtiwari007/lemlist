...
class JobController extends Controller
{
    ...
    public function __construct(JobRepository $jobRepositery,ProjectRepository $projectRepositery,UserRepository $userRepositery)
    {
    ...
    public function index(Request $req){
		...
		if ($req->ajax()) {
		    $get = $req->input();
			 $arrData['get'] = $get;
			$data = $this->jobRepositery->getAll($get);
			return Datatables::of($data)
					...
					->addColumn('action', function($row){
						...
						$reportCrawlerPath = route('api.v1.report-crawler',['id'=>$row->id]);
						...
						$submitCrawler= '<a href="'.$submitCrawlerPath.'" class="btn btn-sm btn-light-success mr-2" title="Job Crawler">Post Your Job</a>';
						$checkUserRole = $this->checkUserRole($row->created_by_user);
						$getPostJobCrawler = $this->jobRepositery->getPostJobCrawler($row->id);
						if(!empty($getPostJobCrawler[0])){
							$reportCrawler= '<a href="javascript:;" class="btn btn-sm btn-light-success mr-2 report-job" data-id="'.$row->id.'" data-href="'.$reportCrawlerPath.'" data-toggle="modal" data-target="#report-modal" title="Report Job Crawler">Report</a>';
						}else{
							$reportCrawler= '';
						}
						if(!empty($checkUserRole) && ($checkUserRole == 'success')){
						    $action = $edit.' '.$view.' '.' '.$csvDownload.' '.$remove.' '.$submitCrawler.' '.$reportCrawler;
						}else{
							$action = $view;
						}
						return $action;
