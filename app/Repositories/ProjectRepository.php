...
class ProjectRepository extends BaseRepository
{
    ...
    public function getAll($extraParams = array()){
       ...
       if(!empty($extraParams['project'])){
			 $projects->where([[DB::raw("name") , 'LIKE', "%{$extraParams['project']}%"]])->orWhere([[DB::raw("description") , 'LIKE', "%{$extraParams['project']}%"]]);
		 }
		 // if(!empty($extraParams['project_desc'])){
			//$projects->where([[DB::raw("description") , 'LIKE', "%{$extraParams['project_desc']}%"]]);
		 // }
		 if(!empty($extraParams['user'])){
			 $projects->where('user_id', $extraParams['user']);
		 }
	...
	public function editProject($data = array(),$userId,$projectId){
			...
			// 			if(!empty($data['old_brand_keyword'][$data['old_url_id'][$k]])){
			// 				    $removeBrandKeyword = ProjectUrlBrandKeyword::where('project_url_id', $data['old_url_id'][$k]);
			// 	                $removeBrandKeyword->delete();
			// 					for($l=0;$l<count($data['old_brand_keyword'][$data['old_url_id'][$k]]);$l++){
			// 						if(!empty($data['old_brand_keyword'][$data['old_url_id'][$k]][$l])){
			// 							$newprojectBrandKeyword = new ProjectUrlBrandKeyword();
			// 							$newprojectBrandKeyword->project_id = $projectId;
			// 							$newprojectBrandKeyword->user_id = $userId;
			// 							$newprojectBrandKeyword->project_url_id = $data['old_url_id'][$k];
			// 							$newprojectBrandKeyword->brand_keyword = $data['old_brand_keyword'][$data['old_url_id'][$k]][$l];
			// 							$newprojectBrandKeyword->save();
			// 						}
									
			// 					}
			// 				}
			// 				if(!empty($data['old_secondary_keyword'][$data['old_url_id'][$k]])){
			// 					$removeSecondaryKeyword = ProjectUrlSecondaryKeyword::where('project_url_id', $data['old_url_id'][$k]);
			// 	                $removeSecondaryKeyword->delete();
			// 					for($m=0;$m<count($data['old_secondary_keyword'][$data['old_url_id'][$k]]);$m++){
			// 						if(!empty($data['old_secondary_keyword'][$data['old_url_id'][$k]][$m])){
			// 							$newprojectSecondaryKeyword = new ProjectUrlSecondaryKeyword();
			// 							$newprojectSecondaryKeyword->project_id = $projectId;
			// 							$newprojectSecondaryKeyword->user_id = $userId;
			// 							$newprojectSecondaryKeyword->project_url_id = $data['old_url_id'][$k];
			// 							$newprojectSecondaryKeyword->secondary_keyword = $data['old_secondary_keyword'][$data['old_url_id'][$k]][$m];
			// 							$newprojectSecondaryKeyword->save();
			// 						}
			...
			// 		if(!empty($data['brand_keyword'][$k])){
			// 			for($l=0;$l<count($data['brand_keyword'][$k]);$l++){
			// 				if(!empty($data['brand_keyword'][$k][$l])){
			// 					$newprojectBrandKeyword = new ProjectUrlBrandKeyword();
			// 					$newprojectBrandKeyword->project_id = $projectId;
			// 					$newprojectBrandKeyword->user_id = $userId;
			// 					$newprojectBrandKeyword->project_url_id = $data['old_url_id'][$k];
			// 					$newprojectBrandKeyword->brand_keyword = $data['brand_keyword'][$k][$l];
			// 					$newprojectBrandKeyword->save();
			// 				}
			...
			// 			for($m=0;$m<count($data['secondary_keyword'][$k]);$m++){
			// 				if(!empty($data['secondary_keyword'][$k][$m])){
			// 					$newprojectSecondaryKeyword = new ProjectUrlSecondaryKeyword();
			// 					$newprojectSecondaryKeyword->project_id = $projectId;
			// 					$newprojectSecondaryKeyword->user_id = $userId;
			// 					$newprojectSecondaryKeyword->project_url_id = $data['old_url_id'][$k];
			// 					$newprojectSecondaryKeyword->secondary_keyword = $data['secondary_keyword'][$k][$m];
			// 					$newprojectSecondaryKeyword->save();
			// 				}
			...
			if(!empty($data['url_name'])){
				...
				for($i=0;$i<count($data['url_name']);$i++){
					if(!empty($data['url_name'][$i])){
						if(!empty($data['old_url_id'][$i])){
                           $pojectUrlId = $data['old_url_id'][$i];
						}else{
							$newprojectUrl = new ProjectUrl();
							$newprojectUrl->project_id = $newProject->id;
							$newprojectUrl->user_id = $userId;
							$newprojectUrl->url_name = $data['url_name'][$i];
							$newprojectUrl->save();
							$pojectUrlId = $newprojectUrl->id;
						}
						if(!empty($data['brand_keyword'][$i])){
							if(!empty($data['old_url_id'][$i])){
							$removeBrandKeyword = ProjectUrlBrandKeyword::where('project_url_id', $data['old_url_id'][$i]);
							$removeBrandKeyword->delete();
							}
								for($l=0;$l<count($data['brand_keyword'][$i]);$l++){
									if(!empty($data['brand_keyword'][$i][$l])){
										...
										$newprojectBrandKeyword->project_id = $projectId;
										$newprojectBrandKeyword->user_id = $userId;
										$newprojectBrandKeyword->project_url_id = $pojectUrlId;
										$newprojectBrandKeyword->brand_keyword = $data['brand_keyword'][$i][$l];
										$newprojectBrandKeyword->save();
									}
							...
							if(!empty($data['secondary_keyword'][$i])){
								if(!empty($data['old_url_id'][$i])){
								$removeSecondaryKeyword = ProjectUrlSecondaryKeyword::where('project_url_id', $data['old_url_id'][$i]);
             	                $removeSecondaryKeyword->delete();
								}
								for($m=0;$m<count($data['secondary_keyword'][$i]);$m++){
									if(!empty($data['secondary_keyword'][$i][$m])){
										...
										$newprojectSecondaryKeyword->project_id = $projectId;
										$newprojectSecondaryKeyword->user_id = $userId;
										$newprojectSecondaryKeyword->project_url_id = $pojectUrlId;
										$newprojectSecondaryKeyword->secondary_keyword = $data['secondary_keyword'][$i][$m];
										$newprojectSecondaryKeyword->save();
									}
	...
	public function getProjectExactKeyword($projectId){
		$objProjectExactKeywords = ProjectExactKeyword::where('project_id',$projectId)->get();
		return $objProjectExactKeywords;
	}
	public function getProjectPartialKeyword($projectId){
		$objProjectExactKeywords = ProjectPartialKeyword::where('project_id',$projectId)->get();
		return $objProjectExactKeywords;
	}
