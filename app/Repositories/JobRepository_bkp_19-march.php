				...
				if(!empty($data['project_url'])){
					for($i=0;$i<count($data['project_url']);$i++){
						if(!empty($data['project_url'][$i])){
							...
							$newjobDetails->project_url_weight = $data['project_url_weight'][$i];
							$newjobDetails->project_url_exact = $data['project_exact'][$i];
							$newjobDetails->project_url_brand_plus_exact = $data['project_brand_plus_exact'][$i];
							$newjobDetails->project_url_secondary_plus_exact = $data['project_secondary_plus_exact'][$i];
							$newjobDetails->project_url_partial_plus_exact = $data['project_partial_plus_exact'][$i];
							$newjobDetails->project_url_partial_plus_brand = $data['project_partial_plus_brand'][$i];
						...
						if(!empty($data['project_url'][$i])){
							...
							if(!empty($getJobDetails[0])){
								foreach($getJobDetails as $jobDetails){
									...
									$updatejobDetails->project_url_weight = $data['project_url_weight'][$i];
									$updatejobDetails->project_url_exact = $data['project_exact'][$i];
									$updatejobDetails->project_url_brand_plus_exact = $data['project_brand_plus_exact'][$i];
									$updatejobDetails->project_url_secondary_plus_exact = $data['project_secondary_plus_exact'][$i];
									$updatejobDetails->project_url_partial_plus_exact = $data['project_partial_plus_exact'][$i];
									$updatejobDetails->project_url_partial_plus_brand = $data['project_partial_plus_brand'][$i];
							...
							foreach($jobs->jobdetails as $jobDetails){
								...
								//var_dump($jobs->projects->partialKeywords->take($arrData['percentageExactKeywordVal'])->toArray());
								 ...
								 for($i=0;$i<round($arrData['percentageSecondaryPlusExact']);$i++){
								  ...
								  // echo "<pre>";var_dump($varTotalListData);
								 ...
								 //echo "<pre>";var_dump($list['data']);exit;
								  for($i=$totalKeywordVal;$i<$arrData['percentageWeightVal'];$i++){
									  $list['data'][] = array(
															 'page_type' => 'GoogleSearchResult',
															 'url' => 'https://www.google.com/',
															 'options.result_url'=> $jobDetails->projectUrls->url_name,
								...
								// echo "<pre>";var_dump($totalKeywordVal);
                                 ...
                                 unset($totalKeywordVal);	
                                 unset($varRoundPercentagePartialPlusbrandVal);								 
								}
						}
						 //All Original data according to jobs 
						// echo "<pre>";var_dump(count($list['data']));var_dump($totalPercentageWeightVal);exit;
						 if(!empty($list['data'])){
							         $k=1;
